#!/usr/bin/env bash
# Publish swift/ to the public SPM distribution mirror github.com/get-encatch/encatch-swift.
#
...
# wholesale with swift/ (minus the exclusion list below) as ONE commit, tagged <version>.
# Development always happens in this monorepo — see swift/DEVELOPMENT.md.
set -euo pipefail
...
if [[ -n "$(git -C "$REPO_ROOT" status --porcelain swift)" ]]; then
  echo "error: uncommitted changes under swift/ — commit or stash first" >&2
  exit 1
fi
