#!/usr/bin/env bash
#
# Reproduces swift/dist/{ios-arm64,sim-arm64}/ — the cinterop-consumable artifact
...
if [[ "$FORCE" -eq 0 ]] && [[ -f "$STAMP_FILE" ]] \
  ...
  if [[ "$(cat "$STAMP_FILE")" == "$CURRENT_HASH" ]]; then
    echo "swift/dist/ is up to date (Sources/ unchanged since last build). Skipping xcodebuild."
    echo "Run with -f/--force to rebuild anyway."
    exit 0
  fi
fi

echo "Rebuilding swift/dist/ ..."
