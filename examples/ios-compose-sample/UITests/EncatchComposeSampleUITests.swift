...
/// Drives the Compose Multiplatform sample app (Variant 4) through init -> show modal form ->
/// show inline form against a running `:mock-server` (see `mock-server/`), attaching a real
/// screenshot at each step. Mirrors `ios-sample/UITests/EncatchSampleUITests.swift` exactly, with
/// accessibility identifiers set via Compose Multiplatform's `Modifier.testTag(...)` (see
/// `compose-sample/src/commonMain/kotlin/com/encatch/composesample/ComposeSampleScreen.kt`), which
/// bridges to UIKit's `accessibilityIdentifier` on iOS. Requires `:mock-server` reachable at
/// `http://127.0.0.1:8089` (or override via the `MOCK_SERVER_BASE_URL` launch environment
/// variable) before running — otherwise `initButton` hits nothing and every subsequent step
/// silently no-ops.
final class EncatchComposeSampleUITests: XCTestCase {
