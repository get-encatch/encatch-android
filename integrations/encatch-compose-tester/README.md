...
## Known gaps (relative to encatch-android-tester/encatch-ios-tester)
...
- **Setup** — API key, default form id, optional API base URL / web host. Saved locally and
  restored on next launch until cleared from Settings.
...
- **Inline** — `EncatchInlineForm` claiming the default form id (exact-match only, see gaps
  above).
...
minus the interceptor step (step 4) and the wildcard-inline step (step 6) — run it on both the
Android app and the [`encatch-compose-tester-ios`](../encatch-compose-tester-ios/README.md) host
to confirm both platforms behave identically off the same shared Compose UI.
