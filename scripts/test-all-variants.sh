...
if ./gradlew :sample-app:assembleDebug :sample-app:assembleDebugAndroidTest \
...
run_android_test() {
    ...
    adb -s "$ANDROID_SERIAL" install -r sample-app/build/outputs/apk/debug/sample-app-debug.apk >/dev/null
    adb -s "$ANDROID_SERIAL" install -r sample-app/build/outputs/apk/androidTest/debug/sample-app-debug-androidTest.apk >/dev/null
...
if MOCK_SERVER_BASE_URL="$IOS_MOCK_BASE_URL" ./ios-sample/test.sh "$IOS_UDID" \
...
run_ios_smoke_test "4-ios-compose-multiplatform" "ios-compose-sample" "EncatchComposeSampleApp" "com.encatch.composesample"
run_ios_smoke_test "5-ios-kmp-sample" "ios-kmp-sample" "EncatchKmpSampleApp" "com.encatch.kmpsample"
