...
Native macOS app for testing the `ios-native` Swift Package under **Mac Catalyst** — a sibling to
...
`ios-native` itself** were needed to run under Catalyst — see
[`ios-native/README.md`](../../ios-native/README.md) for the feasibility findings.
...
- **iOS-only mechanisms dropped, no replacement needed**: the on-screen-keyboard scroll-room
  ...
  anyway.
- This app targets **iOS 17** (not iOS 15 like `ios-native`'s own floor) for `Table`,
