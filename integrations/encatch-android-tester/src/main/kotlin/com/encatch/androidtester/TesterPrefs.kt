...
/** Local persistence for the setup screen, so one APK can be handed to any tester/environment. */
class TesterPrefs(context: Context) {
    private val prefs = context.getSharedPreferences("encatch_tester", Context.MODE_PRIVATE)
...
class TestUsersStore(context: Context) {
    private val prefs = context.getSharedPreferences("encatch_tester_users", Context.MODE_PRIVATE)
