...
/// Drives the KMP host sample app (Variant 5) through init -> show modal form -> show inline
/// form against a running `:mock-server` (see `mock-server/`), attaching a real screenshot at
/// each step. Mirrors `ios-sample/UITests/EncatchSampleUITests.swift` exactly, with accessibility
/// identifiers set directly on the Kotlin/Native UIKit views (see
/// `kmp-sample/src/iosMain/kotlin/com/encatch/kmpsample/KmpSampleViewController.kt`'s
/// `accessibilityIdentifier` assignments). Requires `:mock-server` reachable at
...
/// silently no-ops.
final class EncatchKmpSampleAppUITests: XCTestCase {
