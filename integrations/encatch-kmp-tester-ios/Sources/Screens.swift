    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 20) {
                VStack(spacing: 10) {
                    BrandMark()
                    Text("Encatch KMP Tester").font(.title2.weight(.bold))
...
struct HeaderActions: View {
    ...
    // TesterController reports the theme as an uppercase string ("SYSTEM"/"LIGHT"/"DARK").
    private var themeIcon: String {
        switch state.currentTheme.uppercased() {
        ...
        HStack(spacing: 14) {
            Button(action: { state.cycleTheme() }) {
                HStack(spacing: 4) {
                    ...
                    Text(state.currentTheme.capitalized).font(.caption.weight(.medium))
                }
            }
...
/// Simple left-aligned wrapping layout for preset chips (no `Layout` protocol needed).
...
/// Modal prompting for a locale/country code to switch the SDK to. Sheet-based so it matches
/// the other testers; Apply is disabled while the input is blank.
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 20) {
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    VStack(spacing: 6) {
                        InfoRow(label: "SDK variant", value: "KMP (com.encatch:kmp-sdk, iOS)")
                        Divider()
