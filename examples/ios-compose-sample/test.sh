#!/bin/bash
# Runs EncatchComposeSampleAppUITests against a booted iOS Simulator. Requires :mock-server
# running first (`./gradlew :mock-server:run` from the repo root) — the app defaults to
# http://127.0.0.1:8089, override with MOCK_SERVER_BASE_URL if needed. Also requires
# :compose-sample's debug XCFramework to already be built (see build.sh); this script does not
...
if [ ! -d "$FRAMEWORK" ]; then
    ...
    (cd ../.. && ./gradlew :examples:compose-sample:assembleEncatchComposeSampleDebugXCFramework)
fi
...
xcodebuild test -project EncatchComposeSample.xcodeproj -scheme EncatchComposeSampleApp -destination "id=$DEVICE_ID"
